...
# cf) 두 개 이상의 컬럼 조회 시 , (콜마)로 구분하여 나열
select
...
# : select A from B wher C (A: 컬렴명, B: 테이블명, C: 조건식-연산자)
...
# 시작이 J이고 뒤는 3개인 임의의 문자를 허용 검색: J___
select * from `members`
where
	name like 'J___';
...
# cf) 특정 시간 기준 그 이후의 데이터 조히
select * from `members`
where
	join_date > '2022-01-02';
    
# cf) 날짜나 시간의 특정 부문과 일치하는 데이터 조회
# 날짜, year(컬럼명), month(컬럼명), day(컬럼명)
# 시간: hour(컬럼명), minute(컬렴명), second(컬럼명)
select * from `members`
where
	year(join_date) = '2024';

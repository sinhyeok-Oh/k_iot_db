...
# > 테이블에 존재하는 모든 함수 종속 관계의 결정작 반드시 후보키!

/*
	=== 키(Key) 개념 정리 ===
    1) 후보키(Candidate Key)
    : 테이블의 각 행을 유일하게 식별할 수 있는 최소 속정 집합 
    - 유일성과 최소성을 가짐 
    - 하나의 테이블에서 여러 개의 후보키가 존재할수 있음 
...
# 함수 종속 관계 
# student_id + course + id(복합키) >> score, course_name 결정
# 과목ID(결정자)
# >> course_name 결정 (course_id 결정자이긴 하지만 후보키 아님)
# >> BCNF 위반

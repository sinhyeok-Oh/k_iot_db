...
-- 데이터 무결성: 수동으로 처리할 작업을 자도으로 처리 데이터 오류 방지 (주문 후 결제 테이블 자동 삽입)
...
# cf) DML 이벤트를 감지(insert, updatem, delete) - select X 
...
/*
    ...
    create trigger 트리거명
		트리거시점(종료:after, before) 이벤트종류(insert, update, dalete)
		on 테이블명 
		for each row
...
# - 개발자가 직접 명시적으로 시작
# - COMMIT(적용 시점), ROLLBACK(최소)로 직접 제어 
# > 데이터의 일관성 보장, 오류 복구
# EX) 주분 처리 중 오류 시 전체 작업 롤백(취소)

# CF) 원자성(Atomicitity): 모두 성공 또는 모드 실패 (하나라도 실패 시 모두 롤백)

...
### SQL 종류 ### 
# 1. DDL (Definition): 데이터 정의 언어 (데이터베이스, 데이터베이스 객체 구조 정의)
...
### DCL 사용 목적 ##
# 1. 보안 강화: 민감함 데어터에 접근 가능한 사용자 제한 가능
...
# - '127.0.0.1': 해당 IP 주소를 가진 컴퓨터에서만 로그인 가능 (>> 127.0.0.1: localhose와 동일)
...
### 권한 제거 ###
# 1) 일부 권한 제거
# revoke [궈한 목록] on [데이터베이스].[테이블] from '사용자'@'호스트';

...
# > Entity(객체, 존재하는 것)들의 Relationship(관계)를 나타낸 Diagram(표)
...
# 2) 속성
-- 엔티티의 세부정보, 설명 항목 
-- 일반 속성(이름, 조소, 전화번호 등), 기번키(PK: 회원 ID, 학번 등), 외래키(FK: 주문테이블의 고객ID)
-- 타원형 (속성명은 원 안에 명시)
...
# N:N(다대다) - 여러 엔티티가 서로 다수와 관계 (학생 - 과목: 중간 테이블이 필요)

### ERD 표현 규칙 ###
# 1) 부모 PK를 자식이 FK로 가짐 
# 	> 실선 관계: 자식이 부모의 존재에 종속됨 (부모가 있어야만 자식이 존재)
...
# 주문 상세 내역 테이블) 자식 - 주문ID PK, 주문 상품 ... 등
# >> 관계: 자식 테이블의 기본키가 보모의 기본키를 포함하거나 그대로 사용 (1:1 관계)

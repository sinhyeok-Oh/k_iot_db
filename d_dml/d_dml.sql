...
# selete max(auto_increment컬럼명) from `테이블명`;
select max(id) from `example02`;

# cf) 시작 값 변경
alter table `example02` auto_increment=100; # 태이블 생성 시에도 동일
...
# 맨 아래 Safe Update 옵션 해제(WHERE 조건 제약 X) / 체크(WHERE 조건 제약 O)
# +) 재시작해야 됨
